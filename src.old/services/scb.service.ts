...
export class SCBService {
	...
	static injectInto(m: ng.IModule) {
		m.service("scb", SCBService);
	}

	fetchTotalPopulation(year: number): ng.IPromise<{men: number, women: number}> {
	...
	fetchDeaths(year: number): ng.IPromise<number> {
		const requestData: scb.RequestObject = {
	...
	fetchBirths(year: number): ng.IPromise<number> {
		const requestData: scb.RequestObject = {
			query: [Q_REGION_ALL, Q_TIME(year)],
			response: R_JSON,
		};
	...
	fetchAges(year: number): ng.IPromise<{interval: string, people: number}[]> {
		const requestData: scb.RequestObject = {
	...
	fetchCounties(year: number): ng.IPromise<{county: string, people: number}[]> {
		const requestData: scb.RequestObject = {
			query: [
				{
					code: "Region",
	 ...
	 */
	private post(apiPath: string, request: scb.RequestObject): ng.IPromise<scb.ResponseObject> {
		return this.q(resolve => {
			...
			if (cachedResponse) {
				...
				this.http.post(API_BASE + apiPath, request).then(resp => {
					const data = resp.data as scb.ResponseObject;
